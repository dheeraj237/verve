...
describe('SyncManager.pullWorkspace', () => {
  ...
  it('uses adapter.pullWorkspace when available and saves files', async () => {
    ...
    const workspace = { id: 'ws-1', type: WorkspaceType.GDrive, path: '/', createdAt: new Date().toISOString(), lastAccessed: new Date().toISOString() } as any;
  ...
  it('falls back to listWorkspaceFiles + pull when pullWorkspace missing', async () => {
    ...
    const workspace = { id: 'ws-2', type: WorkspaceType.GDrive, path: '/', createdAt: new Date().toISOString(), lastAccessed: new Date().toISOString() } as any;
